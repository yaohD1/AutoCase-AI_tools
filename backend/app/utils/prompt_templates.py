class PromptTemplates:
    ...
    IMAGE_ANALYSIS_PROMPT = """你是一位资深的UI测试分析师。请分析以下{count}张UI设计图，它们属于同一个功能的多页面截图（按用户操作流程排列），请按页面和模块输出结构化描述。

要求：
1. 逐页分析：对每张图按从上到下、从左到右的顺序，识别该页面的所有功能模块
2. 页面关联：描述页面之间的跳转关系和业务流程（例如：第一张图的列表页点击某行进入第二张图的详情页）
3. 对每个模块描述：
   - 模块名称
...
[
  {
    ...
    IMAGE_ANALYSIS_PROMPT_SINGLE = """你是一位资深的UI测试分析师。请分析这张UI设计图，按模块输出结构化描述。
...
每个模块都要生成用例，按模块自由发挥，不限制数量，但数量不要太夸张，覆盖率高了就可以，确保覆盖核心业务为准：
1. 核心功能路径
2. 边界条件
3. 异常场景
4. UI交互细节
